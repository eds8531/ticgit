...
def six():
    ...
    for card in output:
        card['date'] = card['date'] + timedelta(days = -1)
...
def seven():
    ...
    output1_set = set()
    for card in output:
        if card['term'] in output1_set:
            pass
        else:
            output1.append(card)
        output1_set.add(card['term'])
        
    print(len(output1))
    shelfFile['output'] = output1
    shelfFile.close()
...
def fc():
    ...
    while len(word_list_today) > 0:
        print(f"You have {str(len(word_list_today))} words left in today's list")
        ...
        if correct[0].lower() == 'w':
            ...
            word_list_today.append(card)
        if correct[0].lower() == 'e':
            new_def = input('Please enter an updated definition for this term> ')
            card['definition'] = new_def
            word_list_today.append(card)
        if correct[0].lower() == 'd':
            pass
