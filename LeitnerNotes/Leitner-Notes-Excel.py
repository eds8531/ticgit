...
#A function to give Dropbox support to the update function.
...
def update():
    ...
    import io
    import dropbox
    token = 'sl.Awtrh5rsB3lkOMvlYSD78HIJrfmggbGoPHTLLISwFGiT9jE8LOKZtGRdAMTl94N1lFQDN_oh22C2xwmgLPqbKs14gSxTjBJVyUV04vmRdCi3D8IPQtBNOY0AIWIXW3smrZXXcZQ'
    dbx = dropbox.Dropbox(token)
    yourpath = '/Users/ericschlosser/Desktop/TheHardWay/LeitnerNotes/Leitner-Notes-Excel.py'

    file_stream = stream_dropbox_file(yourpath)
    wb = pd.read_excel(file_stream)
...
def fc():
    ...
    todays_words = input('How many words would you like to learn right now? ')
    todays_words = int(todays_words)
    ...
    for card in word_list:
        if len(word_list_today) < todays_words:
            if card['date'] <= date.today():
